...
// Пример использования где-то в коде:
// const { rank, suit, trumpFlag } = parseCardSafe(someIncomingValue);
...
const GameBoard: React.FC<GameBoardProps> = ({rules = {}}) => {
    ...
    useEffect(() => {
        ...
        const {gameState: initialGameState, cards: initialCards} = initializeGameState(2);
    ...
    useEffect(() => {
        ...
        ws.onmessage = (event) => {
            ...
            if (data.type === 'game_action') {
                ...
                // if (gameState.current === initialGameState) {
                //     const cardsOnTable = data.data.state.table;
                //     console.log('cardsOnTable', cardsOnTable);
                //     if (cardsOnTable.length > 0) {
                //         setTablePairs(() =>
                //             cardsOnTable.map((tableEntry, index) => ({
                //                 attackCardId: tableEntry.attacking ? `table-${index}-attack` : null,
                //                 coverCardId: tableEntry.defending ? `table-${index}-cover` : null,
                //             }))
                //         );
                //
                //         setCards((prevCards) => {
                ...
                //
                //             cardsOnTable.forEach((tableEntry, index) => {
                //                 if (tableEntry.attacking) {
                //                     const {suit, rank, trumpFlag} = parseCard(tableEntry.attacking.card);
                //                     const attackCard = updatedCards.find(c => c.stableId === `table-${index}-attack`);
                //                     if (!attackCard) {
                //                         updatedCards.push({
                //                             stableId: `table-${index}-attack`,
                //                             suit,
                //                             rank,
                //                             trumpFlag,
                //                             location: 'table',
                //                             tablePairIndex: index,
                //                             tableRole: 'attack',
                //                             playerId: Number(tableEntry.attacking.player_id),
                //                         });
                //                     } else {
                //                         attackCard.suit = suit;
                //                         attackCard.rank = rank;
                //                         attackCard.trumpFlag = trumpFlag;
                //                         attackCard.location = 'table';
                //                         attackCard.tablePairIndex = index;
                //                         attackCard.tableRole = 'attack';
                //                         attackCard.playerId = Number(tableEntry.attacking.player_id);
                //                     }
                //                 }
                //
                //                 if (tableEntry.defending) {
                //                     const {suit, rank, trumpFlag} = parseCard(tableEntry.defending);
                //                     const coverCard = updatedCards.find(c => c.stableId === `table-${index}-cover`);
                //                     if (!coverCard) {
                //                         updatedCards.push({
                //                             stableId: `table-${index}-cover`,
                //                             suit,
                //                             rank,
                //                             trumpFlag,
                //                             location: 'table',
                ...
                //                         coverCard.suit = suit;
                //                         coverCard.rank = rank;
                //                         coverCard.trumpFlag = trumpFlag;
                //                         coverCard.location = 'table';
                //                         coverCard.tablePairIndex = index;
                //                         coverCard.tableRole = 'cover';
                //                     }
                //                 }
                //             });
                //
                ...
                //         });
                //         setTablePairs(() => {
                //             const tablePairsArray = Array.from({length: 6}, (_, index) => {
                //                 const tableEntry = cardsOnTable[index] || null;
                //                 return {
                //                     attackCardId: tableEntry?.attacking ? `table-${index}-attack` : null,
                //                     coverCardId: tableEntry?.defending ? `table-${index}-cover` : null,
                //                 };
                //             });
                //             return tablePairsArray;
                //         });
                //
                //
                //     }
    ...
    const handlePlayerCardDrop = (
        ...
        if (roleRef.current === 'attack') {
            ...
            const {
                ...
                attackingCard,
                defendingCard
            } = validateDefense(currentCard, attackCard, foundIndex, cardCenter);
                    ...
                    <React.Fragment key={i}>
                        <div
                            style={{
                                ...
                                width: '50px',
                                height: '70px',
                                backgroundColor: pair.attackCardId ? 'rgba(0,255,0,0.2)' : 'darkgray',
                                border: pair.attackCardId ? '1px dashed green' : '2px dashed white',
                                borderRadius: '5px',
                                pointerEvents: 'none',
                        ...
                        <div
                            style={{
                                ...
                                width: '50px',
                                height: '70px',
                                backgroundColor: pair.coverCardId ? 'rgba(0,255,0,0.2)' : 'transparent',
                                pointerEvents: 'none',
                                transform: 'translate(-50%, -50%)',
                            }}
                        />
